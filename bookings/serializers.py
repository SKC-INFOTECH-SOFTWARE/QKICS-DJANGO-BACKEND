...
class ExpertSlotSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model            = ExpertSlot
        fields           = ["id", "uuid", "expert", "expert_name", "start_datetime", "end_datetime",
                            "duration_minutes", "price", "requires_approval", "is_recurring",
...
class ExpertSlotCreateSerializer(serializers.ModelSerializer):
    class Meta:
        model  = ExpertSlot
        fields = ["start_datetime", "end_datetime", "duration_minutes", "price", "requires_approval"]
    ...
    def validate(self, attrs):
        ...
        if ExpertSlot.objects.filter(expert=expert, status="ACTIVE",
                                     start_datetime__lt=end, end_datetime__gt=start).exists():
...
class ExpertSlotUpdateSerializer(serializers.ModelSerializer):
    class Meta:
        model  = ExpertSlot
        fields = ["start_datetime", "end_datetime", "price", "requires_approval", "status"]
    ...
    def update(self, instance, validated_data):
        if instance.has_active_bookings():
            raise serializers.ValidationError("Cannot modify slot with active bookings.")
        start = validated_data.get("start_datetime", instance.start_datetime)
        end   = validated_data.get("end_datetime",   instance.end_datetime)
        if ExpertSlot.objects.filter(expert=instance.expert, status="ACTIVE",
                                     start_datetime__lt=end, end_datetime__gt=start).exclude(id=instance.id).exists():
            raise serializers.ValidationError("Updated times overlap with an existing slot.")
...
class BookingSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model            = Booking
        fields           = [
            "id", "uuid", "user", "user_name", "expert", "expert_name",
            "slot", "slot_uuid", "status",
            "start_datetime", "end_datetime", "duration_minutes",
            "price", "platform_fee_percent", "platform_fee_amount",
            "expert_earning_amount", "requires_expert_approval",
            "expert_approved_at", "paid_at", "confirmed_at", "completed_at",
            "declined_at", "cancelled_at", "chat_room_id",
            "reschedule_count", "cancellation_reason", "decline_reason",
...
class BookingCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        try:
            slot = ExpertSlot.objects.select_for_update().get(uuid=slot_id, status="ACTIVE")
        except ExpertSlot.DoesNotExist:
        ...
        if Booking.objects.filter(slot=slot, status__in=Booking.ACTIVE_STATUSES).exists():
            raise serializers.ValidationError("Slot already booked.")
        if Booking.objects.filter(user=user, slot=slot, status__in=Booking.ACTIVE_STATUSES).exists():
            raise serializers.ValidationError("You already have a booking for this slot.")
        attrs["slot"] = slot
        return attrs
    ...
    def create(self, validated_data):
        ...
        status = Booking.STATUS_PENDING if slot.requires_approval else Booking.STATUS_AWAITING_PAYMENT
        booking = Booking(
            user=user, expert=slot.expert, slot=slot,
            start_datetime=slot.start_datetime, end_datetime=slot.end_datetime,
            duration_minutes=slot.duration_minutes, price=price,
            requires_expert_approval=slot.requires_approval, status=status,
        )
...
class BookingApprovalSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if booking.status != Booking.STATUS_PENDING:
            raise serializers.ValidationError(f"Cannot approve/decline booking in '{booking.status}' status.")
        if attrs.get("approve"):
            conflict = Booking.objects.filter(
                slot=booking.slot,
                status__in=[Booking.STATUS_AWAITING_PAYMENT, Booking.STATUS_PAID, Booking.STATUS_CONFIRMED],
...
class BookingCancellationSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if not booking.can_be_cancelled():
            raise serializers.ValidationError(f"Cannot cancel booking in '{booking.status}' status.")
...
class BookingStatusUpdateSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if not booking.can_transition_to(new_status):
            raise serializers.ValidationError(f"Cannot transition from '{booking.status}' to '{new_status}'.")
        return attrs
...
class InvestorSlotSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model            = InvestorSlot
        fields           = ["id", "uuid", "investor", "investor_name", "start_datetime", "end_datetime",
                            "duration_minutes", "status", "is_available", "created_at"]
        read_only_fields = fields
...
class InvestorBookingSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields           = ["id", "uuid", "user", "user_name", "investor", "investor_name",
                            "slot", "status", "start_datetime", "end_datetime",
                            "duration_minutes", "reschedule_count", "created_at", "call_room_id"]
        read_only_fields = fields
...
class InvestorBookingCreateSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if not is_user_premium(user):
            raise serializers.ValidationError("Investor consultations are available only for premium users.")
        try:
            slot = InvestorSlot.objects.select_for_update().get(uuid=attrs["slot_id"], status="ACTIVE")
